            ...
            <nav className={(isHome) ? ' z-10 fixed w-screen top-0 py-4' : ' z-10 sticky top-0 py-4 bg-white'}>
                <div className='hidden md:flex justify-center '>
                    <ul className='flex gap-4 rounded-full px-6 py-2 bg-gradient-to-b'>
                        <li className={(path === "/") ? 'hidden' : 'text-zinc-400 hover:text-zinc-200 duration-200 mix-blend-exclusion bg-blend-luminosity text-ellipsis cursor-pointer'}><Link href="/">Home</Link></li>
                        <li className={worksDisplay !== 'hidden' ? 'text-zinc-200 duration-200 mix-blend-exclusion text-ellipsis cursor-pointer' : 'text-zinc-400 hover:text-zinc-200 duration-200 mix-blend-exclusion text-ellipsis cursor-pointer'} onClick={works}>Works</li>
                        <li className={infoDisplay !== 'hidden' ? 'text-zinc-200 duration-200 mix-blend-exclusion text-ellipsis cursor-pointer' : 'text-zinc-400 hover:text-zinc-200 duration-200 mix-blend-exclusion text-ellipsis cursor-pointer'} onClick={info}>Information</li>
                ...
                <Sheet open={isMenuOpen} onOpenChange={setIsMenuOpen}>
                    <SheetTrigger asChild>
                        <Button variant="ghost" size="icon" className="md:hidden bg-white/50 fixed top-8 left-8 z-50">
                            <Menu className="h-6 w-6" />
                            <span className="sr-only">Toggle menu</span>
                        </Button>
                    </SheetTrigger>
                    <SheetContent side="left" className="w-[40%] z-[10000] h-full p-0 bg-transparent border-opacity-0">
                        <div className="grid grid-col w-full h-full text-zinc-200 bg-gray-400 backdrop-filter backdrop-blur-sm bg-opacity-40">
                            <nav className="flex flex-col gap-4 p-4 flex-grow">
                                <Button variant="ghost" asChild onClick={() => setIsMenuOpen(false)} className='flex justify-start'>
                                    <Link href="/">Home</Link>
                                </Button>
                                    ...
                                    <AccordionItem value="works" className=' justify-start'>
                                        ...
                                        <AccordionContent>
                                            <div className="flex flex-col items-start gap-2">
                                                {navbarLinks.works.map((link, idx) => (
                                                    <Button key={idx} variant="ghost" asChild onClick={() => setIsMenuOpen(false)}>
